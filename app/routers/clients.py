...
@clients_router.get("/", response_model=ClientResponse)
...
def update(client_id: int, db:Session = Depends(get_db), user_id: User = Depends(get_current_user)):
    return update_client(db, client_id)
